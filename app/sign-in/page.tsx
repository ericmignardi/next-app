...
export default function SignIn() {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (signIn.status === "complete") {
        ...
        setError(
          `Sign-in status: ${signIn.status}. Additional steps required.`,
        );
      }
    } catch (err) {
      console.error(err);
  ...
  const handleForgotPassword = (e: React.MouseEvent) => {
    e.preventDefault();
    setError(
      "Password recovery is disabled for this custom layout. Please contact admin.",
    );
  };
      ...
      <div className="relative z-10 w-full max-w-md bg-[#090d19]/80 border border-white/[0.03] rounded-2xl p-8 sm:p-10 shadow-2xl backdrop-blur-md">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-xs text-slate-500 font-bold uppercase tracking-wider mt-1 text-center">
            Sign in to view your family archives
          </p>
        </div>
        ...
        <form onSubmit={handleSignIn} className="flex flex-col gap-5">
          <div>
            ...
            <input
              ...
              className="w-full bg-[#111726]/40 border border-white/[0.04] rounded-xl px-4 py-3 text-xs text-white placeholder-slate-600 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/10 transition-all"
            />
          </div>
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              ...
              <button
                ...
                className="text-[11px] font-bold text-blue-500 hover:text-blue-400 transition-colors bg-transparent border-none p-0 cursor-pointer"
              >
                Forgot?
              </button>
            </div>
          ...
          <label className="flex items-center gap-2 text-xs text-slate-400 select-none cursor-pointer self-start">
            <input
              type="checkbox"
              checked={keepSignedIn}
              onChange={(e) => setKeepSignedIn(e.target.checked)}
              className="w-4 h-4 rounded border-white/[0.04] bg-[#111726]/40 text-blue-600 focus:ring-blue-500/20 cursor-pointer accent-blue-600"
            />
